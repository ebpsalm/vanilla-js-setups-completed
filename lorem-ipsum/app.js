...
const textArticle = document.querySelector('.lorem-text');
...
function submitForm(e) {
  ...
  if (isNaN(inputValue) || inputValue <= 0 || inputValue >= 10) {
    textArticle.textContent = text[Math.floor(Math.random() * text.length)];
  } else {
    const articleText = text.slice(0, inputValue);
    const paragraph = articleText
      .map(function (para) {
        return `<p>${para}</p>`;
      })
      .join('');

    textArticle.innerHTML = paragraph;
    inputEl.textContent = '';
  }
}
