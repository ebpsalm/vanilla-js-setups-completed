...
function showFilterBtns() {
  ...
  const showBtns = reducedBtns
    .map(function (btn) {
      return `<button class="filter-btn" type="button" data-id=${btn}>${btn}</button>`;
    })
    .join('');
  btnContainer.innerHTML = showBtns;
  const btns = document.querySelectorAll('.filter-btn');
  btns.forEach(function (btn) {
    btn.addEventListener('click', function (e) {
      const catagory = e.currentTarget.dataset.id;
      const filteredCatagory = menu.filter(function (cat) {
        if (cat.category === catagory) {
          return cat;
        }
      });
      if (catagory === 'all') {
